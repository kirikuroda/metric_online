...
# set a data file
try:
    os.makedirs('./data/behavior/pair2')
except OSError:
    pass
try:
    os.makedirs('./data/log/pair2')
except OSError:
    pass
cwd = os.getcwd()
behavior_file = open(os.path.join(cwd+'/data/behavior/pair2/metric_mri_pair2_behavior_'+expInfo['id']+'_'+expInfo['dateStr']+'.csv'), 'wb')
log_file = open(os.path.join(cwd+'/data/log/pair2/metric_mri_pair2_log_'+expInfo['id']+'_'+expInfo['dateStr']+'.csv'), 'wb')
...
instruction = visual.TextStim(win, text=u'ドットの個数を答えてください。\n※Bさんと同時に取り組むブロックです。', height=60, font='Meiryo')

you_text = visual.TextStim(win, text=u'あなた', height=80, font='Meiryo', pos=(-480, 270))
partner_text = visual.TextStim(win, text=u'Bさん', height=80, font='Meiryo', pos=(480, 270))

'''
eyetracker
'''
edf_file_name = 'sess5_' + str(id) + '.edf'
