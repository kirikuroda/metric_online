...
# set a data file
try:
    os.makedirs('./data/behavior/solo0')
except OSError:
    pass
try:
    os.makedirs('./data/log/solo0')
except OSError:
    pass
cwd = os.getcwd()
behavior_file = open(os.path.join(cwd+'/data/behavior/solo0/metric_mri_behavior_solo0_'+expInfo['id']+'_'+expInfo['dateStr']+'.csv'), 'wb')
log_file = open(os.path.join(cwd+'/data/log/solo0/metric_mri_log_solo0_'+expInfo['id']+'_'+expInfo['dateStr']+'.csv'), 'wb')
...
isi_list = np.random.permutation(list(itertools.chain.from_iterable([[2]*16, [3]*16, [4]*16])))
iti_list = np.random.permutation(list(itertools.chain.from_iterable([[2]*16, [3]*16, [4]*16])))
...
eyetracker
'''
edf_file_name = 'sess1_' + str(id) + '.edf'
